...
type ListResponse = { data: { items: InvoiceRow[]; page: number; pageSize: number; total: number } };

const STATUSES = ["draft", "issued", "paid", "void"] as const;

type SettingsResponse = { data: { defaultCurrencyCode: string } };

export function PrintPressInvoicesClient(props: { tenantSlug: string }) {
  ...
  const loadList = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/printpress/invoices?${params.toString()}`, { cache: "no-store", headers: { "X-Tenant-Id": tenantId } });
  ...
  async function createInvoice() {
    ...
    setErrorKey(null);
    try {
      const res = await apiFetch("/api/printpress/invoices", {
        method: "POST",
      ...
      setModalOpen(false);
      window.location.href = `/t/${props.tenantSlug}/printpress/invoices/${id}`;
    } catch {
      setErrorKey("errors.internal");
    } finally {
      setCreating(false);
    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-card">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            <div className="text-xl font-semibold">{t("app.printpress.invoices.title")}</div>
            <div className="mt-1 text-sm text-gray-700">{t("app.printpress.invoices.subtitle")}</div>
          </div>
          <button
            type="button"
            onClick={openCreate}
          ...
          >
            {t("app.printpress.invoices.action.create")}
          </button>
        </div>

        <div className="mt-6 grid gap-3 lg:grid-cols-3">
          <div className="lg:col-span-2">
            <label className="block text-xs font-medium text-gray-700">{t("app.printpress.invoices.filter.search")}</label>
            <input
              ...
              placeholder={t("app.printpress.invoices.filter.search.placeholder")}
            />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700">{t("app.printpress.invoices.filter.status")}</label>
            <select
            ...
            >
              <option value="">{t("app.printpress.invoices.filter.status.all")}</option>
              {STATUSES.map((s) => (
                <option key={s} value={s}>
                  {t(`app.printpress.invoices.status.${s}`)}
                </option>
              ))}
            </select>
          </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white shadow-card">
        <div className="overflow-x-auto">
          <table className="min-w-[980px] w-full border-separate border-spacing-0">
            <thead>
              <tr>
                <th className="border-b border-gray-200 px-4 py-3 text-left text-sm font-medium text-gray-900">{t("app.printpress.invoices.table.invoice")}</th>
                <th className="border-b border-gray-200 px-4 py-3 text-left text-sm font-medium text-gray-900">{t("app.printpress.invoices.table.customer")}</th>
                <th className="border-b border-gray-200 px-4 py-3 text-left text-sm font-medium text-gray-900">{t("app.printpress.invoices.table.status")}</th>
                <th className="border-b border-gray-200 px-4 py-3 text-left text-sm font-medium text-gray-900">{t("app.printpress.invoices.table.paid")}</th>
                <th className="border-b border-gray-200 px-4 py-3 text-left text-sm font-medium text-gray-900">{t("app.printpress.invoices.table.actions")}</th>
              </tr>
            </thead>
            <tbody className="text-sm">
              {loading ? (
                ...
                <tr>
                  <td className="px-4 py-6 text-gray-600" colSpan={5}>
                    {t("app.printpress.invoices.empty")}
                  </td>
                </tr>
              ) : (
                items.map((row) => (
                  <tr key={row.id}>
                    <td className="border-b border-gray-100 px-4 py-3">
                      <div className="font-medium text-gray-900">{row.invoiceNumber ?? t("app.printpress.invoices.noNumber")}</div>
                    ...
                    <td className="border-b border-gray-100 px-4 py-3 text-gray-700">{t(`app.printpress.invoices.status.${row.status}`)}</td>
                    <td className="border-b border-gray-100 px-4 py-3 text-gray-700">
                      {row.paidTotal} / {row.total} {row.currencyCode}
                    </td>
                    <td className="border-b border-gray-100 px-4 py-3">
                      <Link
                        href={`/t/${props.tenantSlug}/printpress/invoices/${row.id}`}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)}>
        <div className="p-6 md:p-8">
          <div className="flex items-start justify-between gap-4">
            <div className="text-xl font-semibold">{t("app.printpress.invoices.modal.create")}</div>
            <button
              type="button"
