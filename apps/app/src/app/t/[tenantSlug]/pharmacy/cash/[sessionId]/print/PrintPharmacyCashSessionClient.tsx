...
export function PrintPharmacyCashSessionClient(props: { tenantSlug: string; sessionId: string }) {
    ...
    async function load() {
      ...
      try {
        ...
        const [tenantRes, settingsRes, sessionRes] = await Promise.all([
          ...
          apiFetch("/api/pharmacy/settings", { cache: "no-store", headers: { "X-Tenant-Id": membership.tenantId } }),
          apiFetch(`/api/pharmacy/cash-sessions/${props.sessionId}`, { cache: "no-store", headers: { "X-Tenant-Id": membership.tenantId } })
        ]);

        const tenantJson = (await tenantRes.json()) as TenantCurrentResponse;
        const settingsJson = settingsRes.ok ? ((await settingsRes.json()) as PharmacySettingsResponse) : null;
  ...
  if (!tenantId || !tenant || !session) return <div className="rounded-2xl border border-gray-200 bg-white p-6 text-sm text-gray-700">{t("errors.notFound")}</div>;
    ...
    <div className="space-y-4">
      ...
      <div className={["print-root", isThermalPaper ? "print-thermal" : "print-a4"].join(" ")}>
        ...
        <div className="print-paper">
          ...
          <div className={isThermalPaper ? "mt-4 border-t border-dashed border-gray-300 pt-3 text-center text-xs text-gray-600" : "mt-6 text-center text-xs text-gray-600"}>{t("app.shop.print.thanks")}</div>
        </div>
      </div>
    </div>
  );
