...
export function PrintPharmacyLotsClient(props: { tenantSlug: string }) {
  ...
  const [status, setStatus] = useState<"all" | "expired" | "near" | "ok" | "no_expiry">(() => {
    const s = searchParams.get("status");
    if (s === "all" || s === "expired" || s === "near" || s === "ok" || s === "no_expiry") return s;
    return "all";
  });
  const [nearDays, setNearDays] = useState(() => searchParams.get("nearDays")?.trim() ?? "30");
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem(`oneerp.pharmacy.printSettings.${props.tenantSlug}.lots`);
      if (!raw) return;
  ...
  useEffect(() => {
    try {
      localStorage.setItem(`oneerp.pharmacy.printSettings.${props.tenantSlug}.lots`, JSON.stringify({ paper, paddingMm, fontScale, footerText }));
    } catch {}
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        ...
        p.set("status", status);
        p.set("nearDays", nearDays.trim() || "30");
        p.set("pageSize", "100");

        const [tenantRes, locRes] = await Promise.all([
        ...
        if (!tenantRes.ok || !tenantJson.data || !locRes.ok) {
          setErrorKey("errors.internal");
          return;
        }
        ...
        if (!cancelled) {
          ...
          setLocations((locJson.data ?? []).filter((l) => l.isActive).map((l) => ({ id: l.id, name: l.name })));
          setItems(all);
        }
      } catch {
        setErrorKey("errors.internal");
  ...
  }, [tenantId, q, locationId, status, nearDays]);
  ...
  useEffect(() => {
    ...
    async function exportPdf() {
      ...
      try {
        try {
          const threshold = `q=${q.trim() || ""};locationId=${locationId.trim() || ""};status=${status};nearDays=${nearDays.trim() || "30"}`;
          await apiFetch("/api/pharmacy/reports/export-log", {
            method: "POST",
            headers: { "Content-Type": "application/json", "X-Tenant-Id": tenantId },
            body: JSON.stringify({ reportId: "pharmacy.inventory.lots.v1", format: "pdf", locationId: locationId.trim() || undefined, threshold })
          });
        } catch {}
        ...
        const filename = `pharmacy_lots_${safeDate}_${paperParamToFileSuffix(paper)}.pdf`;
        if (isThermal(paper)) {
  ...
  }, [download, paper, pdfExported, q, locationId, status, nearDays, tenantId]);
  ...
  const locName = locationId.trim() ? locations.find((l) => l.id === locationId.trim())?.name ?? locationId.trim() : t("common.all");
      ...
      <div className="no-print rounded-2xl border border-gray-200 bg-white p-4 shadow-card">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-5">
          ...
          <div>
            ...
            <select className="mt-1 h-10 w-full rounded-xl border border-gray-200 bg-white px-3 text-sm" value={locationId} onChange={(e) => setLocationId(e.target.value)} disabled={download}>
          ...
          <div>
            ...
            <select className="mt-1 h-10 w-full rounded-xl border border-gray-200 bg-white px-3 text-sm" value={status} onChange={(e) => setStatus(e.target.value as typeof status)} disabled={download}>
              <option value="all">{t("app.pharmacy.inventory.lots.statusFilter.all")}</option>
              ...
              <option value="ok">{t("app.pharmacy.inventory.lots.status.ok")}</option>
              <option value="no_expiry">{t("app.pharmacy.inventory.lots.status.no_expiry")}</option>
            </select>
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700">{t("app.pharmacy.inventory.lots.filter.nearDays")}</label>
            <input className="mt-1 h-10 w-full rounded-xl border border-gray-200 px-3 text-sm tabular" value={nearDays} onChange={(e) => setNearDays(e.target.value)} disabled={download} />
          </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-1 text-xs text-gray-600">
                    {t("app.pharmacy.export.printTitle.lots")} · {locName}
                  </div>
                </div>
              </div>
            </div>
          ...
          {isThermalPaper ? (
            ...
            <div className="mt-6">
              <table className="w-full border-separate border-spacing-0 text-sm">
                <thead>
                  <tr>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.pharmacy.inventory.lots.table.product")}</th>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.pharmacy.inventory.lots.table.lot")}</th>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.pharmacy.inventory.lots.table.expiry")}</th>
                    <th className="border-b border-gray-200 py-2 text-right font-semibold text-gray-900">{t("app.pharmacy.inventory.lots.table.onHand")}</th>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.pharmacy.inventory.lots.table.status")}</th>
                  </tr>
                </thead>
                <tbody>
                  {items.slice(0, 500).map((r) => (
                    <tr key={r.id}>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-900">
                        {r.product.name}
                        <div className="mt-1 text-xs text-gray-500">{r.product.sku ?? "—"}</div>
                      </td>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-700">{r.lotNumber}</td>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-700">{r.expiryDate ? new Date(r.expiryDate).toISOString().slice(0, 10) : "—"}</td>
                      <td className="border-b border-gray-100 py-2 text-right font-medium text-gray-900 tabular">{r.onHandQty}</td>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-700">{r.status}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
