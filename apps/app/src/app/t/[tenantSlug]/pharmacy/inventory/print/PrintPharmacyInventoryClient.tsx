...
type InventoryItem = { product: { id: string; name: string; sku: string | null; unit: { id: string; name: string; symbol: string | null } | null }; onHandQty: string };
...
export function PrintPharmacyInventoryClient(props: { tenantSlug: string }) {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem(`oneerp.pharmacy.printSettings.${props.tenantSlug}.inventory`);
      if (!raw) return;
  ...
  useEffect(() => {
    try {
      localStorage.setItem(`oneerp.pharmacy.printSettings.${props.tenantSlug}.inventory`, JSON.stringify({ paper, paddingMm, fontScale, footerText }));
    } catch {}
    ...
    async function loadData() {
      ...
      try {
        ...
        const [tenantRes, invRes] = await Promise.all([
          ...
          apiFetch(`/api/pharmacy/inventory?${params.toString()}`, { cache: "no-store", headers: { "X-Tenant-Id": tenantId } })
        ]);
        const tenantJson = (await tenantRes.json()) as TenantCurrentResponse;
        if (!tenantRes.ok || !tenantJson.data || !invRes.ok) {
          setErrorKey("errors.internal");
          return;
        }
        const invJson = (await invRes.json()) as InventoryResponse;
        if (!cancelled) {
          setTenant(tenantJson.data);
          setLocations(invJson.data.locations ?? []);
  ...
  useEffect(() => {
    ...
    async function exportPdf() {
      ...
      try {
        try {
          const threshold = `q=${q.trim() || ""};locationId=${locationId.trim() || ""}`;
          await apiFetch("/api/pharmacy/reports/export-log", {
            method: "POST",
            headers: { "Content-Type": "application/json", "X-Tenant-Id": tenantId },
            body: JSON.stringify({ reportId: "pharmacy.inventory.list.v1", format: "pdf", locationId: locationId.trim() || undefined, threshold })
          });
        } catch {}
        ...
        const filename = `pharmacy_inventory_${safeDate}_${paperParamToFileSuffix(paper)}.pdf`;
        if (isThermal(paper)) {
  ...
  }, [download, paper, pdfExported, q, locationId, tenantId]);
          ...
          <div className={isThermalPaper ? "text-center" : ""}>
            <div className={["flex items-start gap-3", isThermalPaper ? "justify-center" : "justify-between"].join(" ")}>
              ...
              {!isThermalPaper ? (
                <div className="text-right text-sm">
                  <div className="text-xs font-medium text-gray-600">{t("app.pharmacy.export.printTitle.inventory")}</div>
                  <div className="mt-1 text-xs text-gray-600">
                    {t("app.shop.inventory.filter.location")}: <span className="font-medium text-gray-900">{locName}</span>
                  </div>
                </div>
              ) : null}
            </div>
            {isThermalPaper ? (
              <div className="mt-2 text-xs text-gray-600">
                {t("app.pharmacy.export.printTitle.inventory")} · {locName}
              </div>
            ) : null}
          </div>
          ...
          {isThermalPaper ? (
            <div className="mt-4 text-xs">
              <div className="border-t border-dashed border-gray-300 pt-3">
                {items.slice(0, 80).map((row) => (
                  <div key={row.product.id} className="thermal-row">
                    <div className="thermal-row-top">
                      <div className="min-w-0 font-medium text-gray-900">{row.product.name}</div>
                      <div className="shrink-0 text-right font-medium text-gray-900">{row.onHandQty}</div>
                    </div>
                    <div className="mt-1 thermal-row-sub text-gray-600">
                      {row.product.sku ?? "—"}
                      {row.product.unit ? ` · ${row.product.unit.name}${row.product.unit.symbol ? ` (${row.product.unit.symbol})` : ""}` : ""}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="mt-6">
              <table className="w-full border-separate border-spacing-0 text-sm">
                <thead>
                  <tr>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.shop.inventory.table.product")}</th>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.shop.inventory.table.sku")}</th>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.shop.inventory.table.unit")}</th>
                    <th className="border-b border-gray-200 py-2 text-right font-semibold text-gray-900">{t("app.shop.inventory.table.onHand")}</th>
                  </tr>
                </thead>
                <tbody>
                  {items.slice(0, 500).map((row) => (
                    <tr key={row.product.id}>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-900">{row.product.name}</td>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-700">{row.product.sku ?? "—"}</td>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-700">{row.product.unit ? `${row.product.unit.name}${row.product.unit.symbol ? ` (${row.product.unit.symbol})` : ""}` : "—"}</td>
                      <td className="border-b border-gray-100 py-2 text-right font-medium text-gray-900 tabular">{row.onHandQty}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
