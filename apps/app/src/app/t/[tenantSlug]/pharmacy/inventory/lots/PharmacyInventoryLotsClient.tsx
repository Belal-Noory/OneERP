...
export function PharmacyInventoryLotsClient(props: { tenantSlug: string }) {
  ...
  const [status, setStatus] = useState<"all" | "expired" | "near" | "ok" | "no_expiry">("all");
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-card">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            <div className="flex items-center gap-3">
              <Link className="text-sm font-medium text-gray-600 hover:text-gray-900" href={`/t/${props.tenantSlug}/pharmacy/inventory`}>
                {t("app.pharmacy.inventory.lots.back")}
              </Link>
              <div className="text-sm text-gray-400">/</div>
              <div className="text-2xl font-semibold">{t("app.pharmacy.inventory.lots.title")}</div>
            </div>
            <div className="mt-2 text-gray-700">{t("app.pharmacy.inventory.lots.subtitle")}</div>
          </div>
                ...
                <div className="absolute right-0 z-20 mt-2 w-56 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-lg">
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        try {
                          const threshold = `q=${q.trim() || ""};locationId=${locationId || ""};status=${status};nearDays=${nearDays.trim() || "30"}`;
                          await apiFetch("/api/pharmacy/reports/export-log", {
                            method: "POST",
                            headers: { "Content-Type": "application/json", "X-Tenant-Id": tenantId },
                            body: JSON.stringify({ reportId: "pharmacy.inventory.lots.v1", format: "xlsx", locationId: locationId || undefined, threshold })
                          });
                        } catch {}
                        ...
                        const header = ["Product", "SKU", "Lot", "Expiry", "Location", "On hand", "Status", "Days"];
                        ...
                        XLSX.writeFile(wb, `pharmacy_lots_${safeDate}.xlsx`);
                      } catch {
                        setErrorKey("errors.internal");
                      } finally {
                  ...
                  <a className="flex w-full items-center gap-2 px-3 py-2 text-sm text-gray-900 hover:bg-gray-50" href={`/t/${props.tenantSlug}/pharmacy/inventory/lots/print?paper=a4&q=${encodeURIComponent(q.trim())}&locationId=${encodeURIComponent(locationId)}&status=${encodeURIComponent(status)}&nearDays=${encodeURIComponent(nearDays.trim() || "30")}`} target="_blank" rel="noreferrer">
                    {t("app.shop.reports.export.printView")}
                  </a>
                  <a className="flex w-full items-center gap-2 px-3 py-2 text-sm text-gray-900 hover:bg-gray-50" href={`/t/${props.tenantSlug}/pharmacy/inventory/lots/print?paper=a4&download=pdf&q=${encodeURIComponent(q.trim())}&locationId=${encodeURIComponent(locationId)}&status=${encodeURIComponent(status)}&nearDays=${encodeURIComponent(nearDays.trim() || "30")}`} target="_blank" rel="noreferrer">
                    {t("app.shop.reports.export.pdfA4")}
                  </a>
                  <a className="flex w-full items-center gap-2 px-3 py-2 text-sm text-gray-900 hover:bg-gray-50" href={`/t/${props.tenantSlug}/pharmacy/inventory/lots/print?paper=thermal80&download=pdf&q=${encodeURIComponent(q.trim())}&locationId=${encodeURIComponent(locationId)}&status=${encodeURIComponent(status)}&nearDays=${encodeURIComponent(nearDays.trim() || "30")}`} target="_blank" rel="noreferrer">
                    {t("app.shop.reports.export.pdf80")}
                  </a>
                  <a className="flex w-full items-center gap-2 px-3 py-2 text-sm text-gray-900 hover:bg-gray-50" href={`/t/${props.tenantSlug}/pharmacy/inventory/lots/print?paper=thermal58&download=pdf&q=${encodeURIComponent(q.trim())}&locationId=${encodeURIComponent(locationId)}&status=${encodeURIComponent(status)}&nearDays=${encodeURIComponent(nearDays.trim() || "30")}`} target="_blank" rel="noreferrer">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-4">
          ...
          <div>
            ...
            <select className="mt-1 h-11 w-full rounded-xl border border-gray-200 bg-white px-3 text-sm" value={locationId} onChange={(e) => setLocationId(e.target.value)} disabled={!locations.length}>
              {locations.map((l) => (
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                setPage(1);
                setStatus(e.target.value as "all" | "expired" | "near" | "ok" | "no_expiry");
              }}
            >
      ...
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-card">
          <div className="text-xs font-medium text-gray-600">{t("app.pharmacy.inventory.lots.summary.ok")}</div>
          <div className="mt-1 text-2xl font-semibold text-gray-900 tabular">{summary.ok}</div>
        </div>
        <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-card">
          <div className="text-xs font-medium text-gray-600">{t("app.pharmacy.inventory.lots.summary.noExpiry")}</div>
          <div className="mt-1 text-2xl font-semibold text-gray-900 tabular">{summary.noExpiry}</div>
        </div>
      </div>
