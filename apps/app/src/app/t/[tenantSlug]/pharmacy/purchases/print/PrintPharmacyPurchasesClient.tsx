...
export function PrintPharmacyPurchasesClient(props: { tenantSlug: string }) {
  ...
  const [status, setStatus] = useState<"all" | "draft" | "posted" | "void">(() => {
    const s = searchParams.get("status");
    if (s === "all" || s === "draft" || s === "posted" || s === "void") return s;
    return "all";
  });
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem(`oneerp.pharmacy.printSettings.${props.tenantSlug}.purchases`);
      if (!raw) return;
  ...
  useEffect(() => {
    try {
      localStorage.setItem(`oneerp.pharmacy.printSettings.${props.tenantSlug}.purchases`, JSON.stringify({ paper, paddingMm, fontScale, footerText }));
    } catch {}
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        ...
        if (locationId.trim()) baseParams.set("locationId", locationId.trim());
  ...
  useEffect(() => {
    ...
    async function exportPdf() {
      ...
      try {
        try {
          const threshold = `q=${q.trim() || ""};status=${status};locationId=${locationId.trim() || ""}`;
          await apiFetch("/api/pharmacy/reports/export-log", {
            method: "POST",
            headers: { "Content-Type": "application/json", "X-Tenant-Id": tenantId },
            body: JSON.stringify({ reportId: "pharmacy.purchases.list.v1", format: "pdf", locationId: locationId.trim() || undefined, threshold })
          });
        } catch {}
        ...
        const filename = `pharmacy_purchases_${safeDate}_${paperParamToFileSuffix(paper)}.pdf`;
        if (isThermal(paper)) {
  ...
  }, [download, paper, pdfExported, q, status, locationId, tenantId]);
            ...
            <div className={["flex items-start gap-3", isThermalPaper ? "justify-center" : "justify-between"].join(" ")}>
              ...
              {!isThermalPaper ? (
                <div className="text-right text-sm">
                  <div className="text-xs font-medium text-gray-600">{t("app.pharmacy.export.printTitle.purchases")}</div>
                  <div className="mt-1 text-xs text-gray-600">
                    {t("app.pharmacy.purchases.filter.location")}: <span className="font-medium text-gray-900">{locName}</span>
                  </div>
                </div>
              ) : null}
            </div>
            {isThermalPaper ? <div className="mt-2 text-xs text-gray-600">{t("app.pharmacy.export.printTitle.purchases")}</div> : null}
          </div>
          ...
          {isThermalPaper ? (
                ...
                {items.slice(0, 80).map((p) => (
                  <div key={p.id} className="thermal-row">
                    <div className="thermal-row-top">
                      <div className="min-w-0 font-medium text-gray-900">{p.purchaseNumber ?? ""}</div>
                      <div className="shrink-0 text-right font-medium text-gray-900">{formatMoney(p.subtotal, p.currencyCode)}</div>
                    </div>
            ...
            <div className="mt-6">
              <table className="w-full border-separate border-spacing-0 text-sm">
                <thead>
                  <tr>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.pharmacy.purchases.table.number")}</th>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.pharmacy.purchases.table.supplier")}</th>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.pharmacy.purchases.table.location")}</th>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.pharmacy.purchases.table.status")}</th>
                    <th className="border-b border-gray-200 py-2 text-right font-semibold text-gray-900">{t("app.pharmacy.purchases.table.total")}</th>
                  </tr>
                </thead>
                <tbody>
                  {items.slice(0, 500).map((p) => (
                    <tr key={p.id}>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-900">{p.purchaseNumber ?? ""}</td>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-700">{p.supplier?.name ?? "—"}</td>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-700">{p.location.name}</td>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-700">
                        {t(`app.pharmacy.purchases.kind.${p.kind}`)} · {t(`app.pharmacy.purchases.status.${p.status}`)}
                      </td>
                      <td className="border-b border-gray-100 py-2 text-right font-medium text-gray-900 tabular">{formatMoney(p.subtotal, p.currencyCode)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
