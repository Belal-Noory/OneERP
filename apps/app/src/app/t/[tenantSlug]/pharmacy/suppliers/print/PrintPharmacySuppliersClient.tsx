...
type SupplierRow = { id: string; name: string; phone: string | null; email: string | null; status: "active" | "archived"; createdAt: string };
type SuppliersResponse = { data: { items: SupplierRow[]; page: number; pageSize: number; total: number } };
...
export function PrintPharmacySuppliersClient(props: { tenantSlug: string }) {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem(`oneerp.pharmacy.printSettings.${props.tenantSlug}.suppliers`);
      if (!raw) return;
  ...
  useEffect(() => {
    try {
      localStorage.setItem(`oneerp.pharmacy.printSettings.${props.tenantSlug}.suppliers`, JSON.stringify({ paper, paddingMm, fontScale, footerText }));
    } catch {}
      ...
      try {
        try {
          ...
          await apiFetch("/api/pharmacy/reports/export-log", {
            ...
            body: JSON.stringify({ reportId: "pharmacy.suppliers.list.v1", format: "pdf", threshold })
          });
        } catch {}
        ...
        const filename = `pharmacy_suppliers_${safeDate}_${paperParamToFileSuffix(paper)}.pdf`;
        if (isThermal(paper)) {
            ...
            <div className={["flex items-start gap-3", isThermalPaper ? "justify-center" : "justify-between"].join(" ")}>
              ...
              {!isThermalPaper ? (
                <div className="text-right text-sm">
                  <div className="text-xs font-medium text-gray-600">{t("app.pharmacy.export.printTitle.suppliers")}</div>
                  <div className="mt-1 text-xs text-gray-600">
                    {t("app.pharmacy.suppliers.filter.status")}: <span className="font-medium text-gray-900">{t(`app.pharmacy.suppliers.status.${status}`)}</span>
                  </div>
                </div>
              ) : null}
            </div>
            {isThermalPaper ? <div className="mt-2 text-xs text-gray-600">{t("app.pharmacy.export.printTitle.suppliers")}</div> : null}
          </div>
          ...
          {isThermalPaper ? (
            <div className="mt-4 text-xs">
              <div className="border-t border-dashed border-gray-300 pt-3">
                {items.slice(0, 80).map((s) => (
                  <div key={s.id} className="thermal-row">
                    <div className="thermal-row-top">
                      <div className="min-w-0 font-medium text-gray-900">{s.name}</div>
                      <div className="shrink-0 text-right font-medium text-gray-900">{s.status}</div>
                    </div>
            ...
            <div className="mt-6">
              <table className="w-full border-separate border-spacing-0 text-sm">
                <thead>
                  <tr>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.pharmacy.suppliers.table.name")}</th>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.pharmacy.suppliers.table.phone")}</th>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.pharmacy.suppliers.table.email")}</th>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.pharmacy.suppliers.table.status")}</th>
                  </tr>
                </thead>
                <tbody>
                  {items.slice(0, 500).map((s) => (
                    <tr key={s.id}>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-900">{s.name}</td>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-700">{s.phone ?? "—"}</td>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-700">{s.email ?? "—"}</td>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-700">{t(`app.pharmacy.suppliers.status.${s.status}`)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
