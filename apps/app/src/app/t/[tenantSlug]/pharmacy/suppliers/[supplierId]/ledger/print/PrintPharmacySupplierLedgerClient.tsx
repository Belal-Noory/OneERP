...
type SupplierResponse = { data: { id: string; name: string } };
...
export function PrintPharmacySupplierLedgerClient(props: { tenantSlug: string; supplierId: string }) {
  ...
  const [from, setFrom] = useState(() => searchParams.get("from")?.trim() ?? "");
  const [to, setTo] = useState(() => searchParams.get("to")?.trim() ?? "");
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem(`oneerp.pharmacy.printSettings.${props.tenantSlug}.supplierLedger`);
      if (!raw) return;
  ...
  useEffect(() => {
    try {
      localStorage.setItem(`oneerp.pharmacy.printSettings.${props.tenantSlug}.supplierLedger`, JSON.stringify({ paper, paddingMm, fontScale, footerText }));
    } catch {}
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        ...
        const [tenantRes, sRes, lRes] = await Promise.all([
          ...
          apiFetch(`/api/pharmacy/suppliers/${props.supplierId}/ledger?${p.toString()}`, { cache: "no-store", headers: { "X-Tenant-Id": tenantId } })
        ]);
        const tenantJson = (await tenantRes.json()) as TenantCurrentResponse;
        const sJson = (await sRes.json()) as SupplierResponse;
        const lJson = (await lRes.json()) as LedgerResponse;
        if (!tenantRes.ok || !tenantJson.data || !sRes.ok || !lRes.ok) {
          setErrorKey("errors.internal");
          return;
        }
        if (!cancelled) {
          setTenant(tenantJson.data);
          setSupplier(sJson.data);
          setLedger(lJson.data);
        }
      } catch {
        setErrorKey("errors.internal");
      } finally {
  ...
  }, [tenantId, props.supplierId, from, to]);
      ...
      try {
        try {
          ...
          await apiFetch("/api/pharmacy/reports/export-log", {
            ...
            body: JSON.stringify({ reportId: "pharmacy.supplierLedger.v1", format: "pdf", threshold })
          });
        } catch {}
        ...
        const filename = `pharmacy_supplier_ledger_${safeDate}_${paperParamToFileSuffix(paper)}.pdf`;
        if (isThermal(paper)) {
  ...
  }, [download, paper, pdfExported, from, to, tenantId, props.supplierId]);
  ...
  if (!tenant || !supplier || !ledger) return <div className="rounded-2xl border border-red-200 bg-red-50 p-6 text-sm text-red-700">{t("errors.internal")}</div>;
      ...
      <div className="no-print rounded-2xl border border-gray-200 bg-white p-4 shadow-card">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-3">
          <div>
            ...
            <input className="mt-1 h-10 w-full rounded-xl border border-gray-200 px-3 text-sm" value={from} onChange={(e) => setFrom(e.target.value)} placeholder="YYYY-MM-DD" disabled={download} />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700">{t("app.pharmacy.supplierLedger.filter.to")}</label>
            <input className="mt-1 h-10 w-full rounded-xl border border-gray-200 px-3 text-sm" value={to} onChange={(e) => setTo(e.target.value)} placeholder="YYYY-MM-DD" disabled={download} />
          </div>
          <div>
      ...
      <div className={["print-root", isThermalPaper ? "print-thermal" : "print-a4"].join(" ")} style={cssVars}>
        ...
        <div className="print-paper">
          <div className={isThermalPaper ? "text-center" : ""}>
            <div className={["flex items-start gap-3", isThermalPaper ? "justify-center" : "justify-between"].join(" ")}>
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-1 text-xs text-gray-600">{supplier.name}</div>
                </div>
              </div>
              {!isThermalPaper ? (
                <div className="text-right text-sm">
                  <div className="text-xs font-medium text-gray-600">{t("app.pharmacy.export.printTitle.supplierLedger")}</div>
            ...
            {isThermalPaper ? <div className="mt-2 text-xs text-gray-600">{t("app.pharmacy.export.printTitle.supplierLedger")}</div> : null}
          </div>
          ...
          {isThermalPaper ? (
              ...
              <div className="border-t border-dashed border-gray-300 pt-3">
                <div className="thermal-row">
                  <div className="thermal-row-top">
                    <div className="min-w-0 font-medium text-gray-900">{t("app.pharmacy.supplierLedger.opening")}</div>
                    <div className="shrink-0 text-right font-medium text-gray-900">{ledger.openingBalance}</div>
                  </div>
                </div>
                {ledger.items.slice(0, 60).map((i, idx) => (
                  <div key={`${i.time}:${idx}`} className="thermal-row">
                    <div className="thermal-row-top">
                      <div className="min-w-0 font-medium text-gray-900">{i.ref}</div>
                      <div className="shrink-0 text-right font-medium text-gray-900">{i.balance}</div>
                    </div>
                    <div className="mt-1 thermal-row-sub text-gray-600">
                      {i.type} · {new Date(i.time).toLocaleString()}
                    </div>
                  </div>
                ))}
                <div className="thermal-row">
                  <div className="thermal-row-top">
                    <div className="min-w-0 font-medium text-gray-900">{t("app.pharmacy.supplierLedger.closing")}</div>
                    <div className="shrink-0 text-right font-medium text-gray-900">{ledger.closingBalance}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6">
              <table className="w-full border-separate border-spacing-0 text-sm">
                <thead>
                  <tr>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.pharmacy.supplierLedger.table.time")}</th>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.pharmacy.supplierLedger.table.type")}</th>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.pharmacy.supplierLedger.table.ref")}</th>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.pharmacy.supplierLedger.table.method")}</th>
                    <th className="border-b border-gray-200 py-2 text-right font-semibold text-gray-900">{t("app.pharmacy.supplierLedger.table.debit")}</th>
                    <th className="border-b border-gray-200 py-2 text-right font-semibold text-gray-900">{t("app.pharmacy.supplierLedger.table.credit")}</th>
                    <th className="border-b border-gray-200 py-2 text-right font-semibold text-gray-900">{t("app.pharmacy.supplierLedger.table.balance")}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td className="border-b border-gray-100 py-2 pr-3 text-gray-700" colSpan={4}>
                      {t("app.pharmacy.supplierLedger.opening")}
                    </td>
                    <td className="border-b border-gray-100 py-2 text-right text-gray-700">—</td>
                    <td className="border-b border-gray-100 py-2 text-right text-gray-700">—</td>
                    <td className="border-b border-gray-100 py-2 text-right font-medium text-gray-900 tabular">{ledger.openingBalance}</td>
                  </tr>
                  {ledger.items.slice(0, 500).map((i, idx) => (
                    <tr key={`${i.time}:${idx}`}>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-700">{new Date(i.time).toLocaleString()}</td>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-700">{i.type}</td>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-700">{i.ref}</td>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-700">{i.method ?? "—"}</td>
                      <td className="border-b border-gray-100 py-2 text-right text-gray-700 tabular">{i.debit}</td>
                      <td className="border-b border-gray-100 py-2 text-right text-gray-700 tabular">{i.credit}</td>
                      <td className="border-b border-gray-100 py-2 text-right font-medium text-gray-900 tabular">{i.balance}</td>
                    </tr>
                  ))}
                  <tr>
                    <td className="py-2 text-gray-700" colSpan={6}>
                      {t("app.pharmacy.supplierLedger.closing")}
                    </td>
                    <td className="py-2 text-right font-medium text-gray-900 tabular">{ledger.closingBalance}</td>
                  </tr>
                </tbody>
              </table>
              {ledger.items.length > 500 ? <div className="mt-3 text-center text-xs text-gray-600">…</div> : null}
            </div>
          )}
