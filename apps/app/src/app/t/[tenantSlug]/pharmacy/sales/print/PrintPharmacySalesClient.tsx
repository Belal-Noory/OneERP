...
export function PrintPharmacySalesClient(props: { tenantSlug: string }) {
  ...
  const [status, setStatus] = useState<"all" | "draft" | "posted" | "void">(() => {
    ...
    if (s === "draft" || s === "posted" || s === "void" || s === "all") return s;
    return "posted";
  });
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem(`oneerp.pharmacy.printSettings.${props.tenantSlug}.sales`);
      if (!raw) return;
  ...
  useEffect(() => {
    try {
      localStorage.setItem(`oneerp.pharmacy.printSettings.${props.tenantSlug}.sales`, JSON.stringify({ paper, paddingMm, fontScale, footerText }));
    } catch {}
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        ...
        const [tenantRes] = await Promise.all([apiFetch("/api/tenants/current", { cache: "no-store", headers: { "X-Tenant-Id": tenantId } })]);
        ...
        for (let p = 1; p <= 60; p += 1) {
          ...
          const res = await apiFetch(`/api/pharmacy/invoices?${pageParams.toString()}`, { cache: "no-store", headers: { "X-Tenant-Id": tenantId } });
      ...
      try {
        try {
          ...
          await apiFetch("/api/pharmacy/reports/export-log", {
            ...
            body: JSON.stringify({ reportId: "pharmacy.sales.list.v1", format: "pdf", threshold })
          });
        } catch {}
        ...
        const filename = `pharmacy_sales_${safeDate}_${paperParamToFileSuffix(paper)}.pdf`;
        if (isThermal(paper)) {
  ...
  }, [download, paper, pdfExported, q, status, tenantId]);
            ...
            <div className={["flex items-start gap-3", isThermalPaper ? "justify-center" : "justify-between"].join(" ")}>
              ...
              {!isThermalPaper ? (
                <div className="text-right text-sm">
                  <div className="text-xs font-medium text-gray-600">{t("app.pharmacy.export.printTitle.sales")}</div>
                  <div className="mt-1 text-xs text-gray-600">
                    {t("app.pharmacy.sales.filter.status")}: <span className="font-medium text-gray-900">{status}</span>
                  </div>
                </div>
              ) : null}
            </div>
            {isThermalPaper ? <div className="mt-2 text-xs text-gray-600">{t("app.pharmacy.export.printTitle.sales")}</div> : null}
          </div>

          {isThermalPaper ? (
            <div className="mt-4 text-xs">
              <div className="border-t border-dashed border-gray-300 pt-3">
                {items.slice(0, 80).map((inv) => (
                  <div key={inv.id} className="thermal-row">
                    <div className="thermal-row-top">
                      <div className="min-w-0 font-medium text-gray-900">{inv.invoiceNumber ?? ""}</div>
                      <div className="shrink-0 text-right font-medium text-gray-900">{formatMoney(inv.subtotal, inv.currencyCode)}</div>
                    </div>
            ...
            <div className="mt-6">
              <table className="w-full border-separate border-spacing-0 text-sm">
                <thead>
                  <tr>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.pharmacy.sales.table.number")}</th>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.pharmacy.sales.table.customer")}</th>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.pharmacy.sales.table.location")}</th>
                    <th className="border-b border-gray-200 py-2 text-left font-semibold text-gray-900">{t("app.pharmacy.sales.table.status")}</th>
                    <th className="border-b border-gray-200 py-2 text-right font-semibold text-gray-900">{t("app.pharmacy.sales.table.subtotal")}</th>
                  </tr>
                </thead>
                <tbody>
                  {items.slice(0, 500).map((inv) => (
                    <tr key={inv.id}>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-900">{inv.invoiceNumber ?? ""}</td>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-700">{inv.customer?.name ?? "—"}</td>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-700">{inv.location?.name ?? "—"}</td>
                      <td className="border-b border-gray-100 py-2 pr-3 text-gray-700">
                        {t(`app.pharmacy.sales.kind.${inv.kind}`)} · {t(`app.pharmacy.sales.status.${inv.status}`)}
                      </td>
                      <td className="border-b border-gray-100 py-2 text-right font-medium text-gray-900 tabular">{formatMoney(inv.subtotal, inv.currencyCode)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
