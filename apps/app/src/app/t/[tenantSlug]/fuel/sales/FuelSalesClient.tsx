...
export function FuelSalesClient(props: { tenantSlug: string }) {
  ...
  async function loadData() {
    ...
    if (!salesRes.ok || !pumpsRes.ok || !shiftsRes.ok) {
      try {
        const e = !salesRes.ok ? await salesRes.json() : !pumpsRes.ok ? await pumpsRes.json() : await shiftsRes.json();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white">
        <table className="w-full text-left text-sm"><thead className="bg-gray-50 text-gray-600"><tr><th className="px-4 py-3">{t("app.fuel.sales.table.date")}</th><th className="px-4 py-3">{t("app.fuel.sales.table.nozzle")}</th><th className="px-4 py-3">{t("app.fuel.sales.table.volume")}</th><th className="px-4 py-3">{t("app.fuel.sales.table.total")}</th><th className="px-4 py-3">{t("app.fuel.sales.table.payment")}</th>{canCreate && <th className="px-4 py-3">{t("app.fuel.tanks.table.actions")}</th>}</tr></thead><tbody className="divide-y divide-gray-100">{sales.length === 0 ? <tr><td colSpan={canCreate ? 6 : 5} className="px-4 py-8 text-center text-gray-500">{t("app.fuel.sales.empty")}</td></tr> : sales.map((s) => <tr key={s.id}><td className="px-4 py-3">{new Date(s.createdAt).toLocaleString()}</td><td className="px-4 py-3">{s.nozzle.name}</td><td className="px-4 py-3">{s.volume}</td><td className="px-4 py-3">{s.totalAmount}</td><td className="px-4 py-3">{s.paymentMethod}</td>{canCreate && <td className="px-4 py-3"><div className="flex items-center gap-2"><button type="button" className="text-xs text-gray-700 hover:text-gray-900" onClick={() => { setEditingId(s.id); setNozzleId((s as { nozzleId?: string }).nozzleId || ""); setVolume(s.volume); setPricePerUnit(s.pricePerUnit); setPaymentMethod(s.paymentMethod); setModalOpen(true); }}>{t("common.button.edit")}</button><button type="button" className="text-xs text-red-600 hover:text-red-700" onClick={() => void removeSale(s.id)}>{t("common.button.remove")}</button></div></td>}</tr>)}</tbody></table>
      </div>
