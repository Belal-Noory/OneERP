...
export function ShopTabs(props: { tenantSlug: string }) {
  ...
  useEffect(() => {
    ...
    async function tick() {
      ...
      try {
        const s2 = await w.oneerp.getOfflineModuleStatus({ moduleId: "shop" });
  ...
  const tabs = useMemo(
    () => [
      { key: "overview", href: base, label: t("app.shop.tab.overview") },
      { key: "pos", href: `${base}/pos`, label: t("app.shop.tab.pos") },
      { key: "cash", href: `${base}/cash`, label: t("app.shop.tab.cash") },
      { key: "purchases", href: `${base}/purchases`, label: t("app.shop.tab.purchases") },
      { key: "suppliers", href: `${base}/suppliers`, label: t("app.shop.tab.suppliers") },
      { key: "products", href: `${base}/products`, label: t("app.shop.tab.products") },
      { key: "labels", href: `${base}/labels`, label: t("app.shop.tab.labels") },
      { key: "customers", href: `${base}/customers`, label: t("app.shop.tab.customers") },
      { key: "orders", href: `${base}/orders`, label: t("app.shop.tab.orders") },
      { key: "inventory", href: `${base}/inventory`, label: t("app.shop.tab.inventory") },
      { key: "reports", href: `${base}/reports`, label: t("app.shop.tab.reports") }
    ],
    [base, t]
  );
  ...
  const activeKey = useMemo(() => {
    ...
    if (pathname.startsWith(`${base}/suppliers`)) return "suppliers";
    if (pathname.startsWith(`${base}/products`)) return "products";
    if (pathname.startsWith(`${base}/labels`)) return "labels";
    ...
    <>
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          ...
          try {
            await w.oneerp.syncModule({ moduleId: "shop" });
      ...
      <OfflineSyncErrorsDialog open={errorsOpen} onClose={() => setErrorsOpen(false)} moduleId="shop" />
    </>
  );
}
